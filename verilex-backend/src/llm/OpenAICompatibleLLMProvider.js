...
export const SCENARIO_SYSTEM_PROMPT = `You convert a lay description of a real-world situation into a structured JSON representation used ONLY for legal information retrieval.
...
export const EXPLANATION_SYSTEM_PROMPT = `You are the explanation layer of a legal-research tool for India. You explain how ONE retrieved legal source relates to a user's described situation.
...
/**
 * Shared transport + prompt-building for any OpenAI-chat-completions-shaped
 * API (OpenAI itself, OpenRouter, or any other compatible gateway).
 * Subclasses just supply name/apiKey/baseUrl/model/extraHeaders — the
 * prompts and grounding rules never change per provider.
 */
export class OpenAICompatibleLLMProvider extends LLMProvider {
  constructor({ apiKey, baseUrl, model, extraHeaders = {}, requiredEnvHint }) {
    super();
    if (!apiKey) throw new Error(`${requiredEnvHint || 'An API key'} is required for this LLM provider`);
    this.apiKey = apiKey;
  ...
  async _chatJson(system, user) {
    ...
    if (!content) throw new Error(`${this.name} API returned no content`);
