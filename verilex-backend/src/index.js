...
app.get('/api/health', (_req, res) => {
  res.json({
    status: 'ok',
    legalProviders: getActiveProviders().map((p) => p.name),
...
app.listen(config.port, () => {
  const providerNames = getActiveProviders().map((p) => p.name).join(' + ');
  console.log(`Verilex backend listening on port ${config.port} (providers: ${providerNames})`);
});
